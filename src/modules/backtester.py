...
from modules.risk import (
    sharpe_ratio_one_asset,
    calmar_ratio_one_asset,
    sortino_ratio_one_asset,
)
...
def long_only_backtester(
    ...
    print(
        f"Sharpe ratio: {sharpe_ratio_one_asset(trades_df['return'], buy_and_hold_return/100):.2f} (risk free rate = buy and hold)"
    )
    print(
        f"Sortino ratio: {sortino_ratio_one_asset(trades_df['return'], buy_and_hold_return/100):.2f} (risk free rate = buy and hold)"
    )
    print(
        f"Calmar ratio: {calmar_ratio_one_asset(trades_df['return']):.2f} (risk free rate = buy and hold)"
    )
