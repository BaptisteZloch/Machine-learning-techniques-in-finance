# FROM : https://github.com/manitadayon/Auto_HMM
...
class DHMM_Model:
    ...
    def Dstate_sorting(self):
        ...
        last_col_sorted = last_col[
            :
        ]  # Another copy of the list to compare the sorted and normal list
        last_col_sorted.sort(
            reverse=True
        )  # since the higher probability the better(Remember the last column corresponds to the best observations.)
    ...
    def AIC_BIC(self):
        ...
        for ii in range(2, self.Max_state + 1):
            ...
            self.num_params = (
                ii * (ii - 1) + ii * (self.N_symb - 1) + (ii - 1)
            )  # these parameters can be automatically selected
            # based on transmat_ and emission_prob_. In this case the self.num_params will be used after Model.

            Model = MultinomialHMM(n_components=ii, tol=pow(10, -5)).fit(
                self.Train_Data, self.Len
            )
            AIC.append(-2 * Model.score(self.Train_Data) + 2 * self.num_params)
            BIC.append(
                -2 * Model.score(self.Train_Data)
                + self.num_params * np.log(self.Train_Data.shape[0])
            )

        Temp1 = np.argmin(AIC)
        opt_state = (
            Temp1 + 2
        )  # This is because 1 state is not possible and if np.argmin is equal to 0 this means we have 2 states.
    ...
    def Best_BIC(self):
        ...
        self.BEST.append(
            Hold_state[Hold_bic.index(min(Hold_bic))]
    ...
    def Viterbi_list(self):  # Test_data is list of numpy arrays.
        ...
        mapping = (
            self.Dstate_sorting()
        )  # what this one does is the order of elements in descending order
    ...
    def Best_States(self):
        ...
        self.Test_data = np.array(
            self.Data.iloc[int(self.N * self.Train_ratio) : self.N, :]
        )

        self.Len = [
            self.T for ii in range(0, self.Data_train.shape[0])
        ]  # Lengths must be list
        self.Train_Data = np.array(self.Data_train).reshape(
            (-1, 1)
        )  # Convert to numpy array with one column
        ...
        self.Model = MultinomialHMM(
            n_components=int(self.BEST[0]), tol=pow(10, -5)
        ).fit(self.Train_Data, self.Len)
...
class Supervised_DHMM(DHMM_Model):
    def AIC_BIC(self):
        ...
        self.num_params = (
            ...
            + self.Max_state * (self.N_symb - 1)
            + (self.Max_state - 1)
        )  # these parameters can be automatically selected
        Model = MultinomialHMM(n_components=self.Max_state, tol=pow(10, -5)).fit(
            self.Train_Data, self.Len
        )
        AIC.append(-2 * Model.score(self.Train_Data) + 2 * self.num_params)
        BIC.append(
